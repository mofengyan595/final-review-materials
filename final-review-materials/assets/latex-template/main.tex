...
% 先完成 coverage/ 下的来源清单、逐页覆盖矩阵、例题目录和课程图，再根据真实资料填写课程框架、考试范围、教师重点、章节关系和复习顺序。
...
写成面向零基础的完整讲解：先补前置知识和问题背景，再给直观理解、正式定义、原理或推导、公式符号与适用条件、课件例题、图表观察方法、易错点、自测和考试问法。简单知识点可以合并表达，不要机械堆空标题。

\subsubsection{课件例题}

把教师课件中的例题放在对应知识点下，保留原题、必要图片和来源，先说明它考查什么，再写完整步骤和答案。明确区分教师原解与推导解答，不要把推导答案伪装成课件答案。
